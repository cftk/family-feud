...
var app = {
  ...
  makeQuestion: function (qNum) {
    ...
    for (var i = 0; i < qNum; i++) {
      var aLI
      if (qAnswr[i]) {
        aLI = $("<div class='cardHolder' id='" + (i + 1) + "'>" +
                  "<div class='card'>" +
                    "<div class='front'>" +
...
$(document).keypress(function (e) {
  ...
  if (key === 'x') {
    var wrong = new Audio('./wrong.m4a')
    wrong.play()
    var wrongAnswer = '<div class="wrongAnswerHolder">'
    for (var i = 0; i < counter; i++) {
      wrongAnswer += '<img height="300px" src="./wrong_indicator.png" id="wrongIndicator" />'
    }
    wrongAnswer += '</div>'
    $('body').append(wrongAnswer)
    setTimeout(function () {
      $('.wrongAnswerHolder').remove()
    }, 1500)
